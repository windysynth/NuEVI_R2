...
static void mainTitleGetStr(char *out)
{
  ...
  if (batteryType < 3)
  { // batt is not LiPo with revised wiring
    memcpy(splice1, (vMeterReading > vFullLimit) ? "USB" : "BAT", 3);
    if (vMeterReading <= vLowLimit)
    { // 2300 alkaline, 2250 lipo, 2200 nimh
  ...
  { // batt is LiPo with revised wiring
...
const MenuEntrySub octaveMenu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(OCTAVE_ADDR, octave); }, nullptr};
...
const MenuEntrySub trill3Menu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(TRILL3_INTERVAL_ADDR, trill3_interval); }, nullptr};
...
const MenuEntrySub dacModeMenu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(DAC_MODE_ADDR, dacMode); }, nullptr};
...
const MenuEntrySub cvTuneMenu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(CVTUNE_ADDR, cvTune); }, nullptr};
...
const MenuEntrySub cvScaleMenu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(CVSCALE_ADDR, cvScale); }, nullptr};
...
const MenuEntrySub wlPowerMenu = {
    ...
    [](SubMenuRef __unused)
    { sendWLPower(wireless_power); }, nullptr};
...
const MenuEntrySub wlChannelMenu = {
    ...
    [](SubMenuRef __unused)
    { sendWLChannel(wireless_channel); }, nullptr};
...
const MenuEntrySub breathCC2RiseMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(BREATH_CC2_RISE_ADDR, breathCC2Rise); }, nullptr};
...
const MenuEntrySub brHarmonicsMenu = {
    ...
    {
      if (brHarmSetting)
        numToString(brHarmSetting, out);
      else
        strncpy(out, "OFF", 4);
    },
    [](const MenuEntrySub &__unused sub)
    { writeSetting(BRHARMSET_ADDR, brHarmSetting); }, nullptr};
...
const MenuEntrySub velocityMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VELOCITY_ADDR, velocity); }, nullptr};
...
const MenuEntrySub velSmpDlMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VEL_SMP_DL_ADDR, velSmpDl); }, nullptr};
...
const MenuEntrySub velBiasMenu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(VEL_BIAS_ADDR, velBias); }, nullptr};
...
const MenuEntrySub breathIntervalMenu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(BRINTERV_ADDR, breathInterval); }, nullptr};
...
const MenuEntrySub biteCtlMenu = {
    ...
    [](SubMenuRef __unused, char *out, const char **__unused unit)
    {
      const char *labs[] = {"OFF", "VIB", "GLD", "CC", "GLO"}; // ws: added GLO for glissando
      strncpy(out, labs[biteControl], 4);
    },
    [](SubMenuRef __unused sub)
    { writeSetting(BITECTL_ADDR, biteControl); }, nullptr};
...
const MenuEntrySub biteCCMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(BITECC_ADDR, biteCC); }, nullptr};
...
const MenuEntrySub leverCtlMenu = {
    ...
    [](SubMenuRef __unused sub)
    { writeSetting(LEVERCTL_ADDR, leverControl); }, nullptr};
...
const MenuEntrySub leverCCMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(LEVERCC_ADDR, leverCC); }, nullptr};
...
const MenuEntrySub stripCtlMenu = {
    ...
    [](SubMenuRef __unused sub)
    { writeSetting(STRIPCTL_ADDR, stripControl); }, nullptr};
...
const MenuEntrySub stripCCMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(STRIPCC_ADDR, stripCC); }, nullptr};
...
const MenuEntrySub portMenu = {
    ...
    [](SubMenuRef __unused sub)
    { writeSetting(PORTAM_ADDR, portamento); }, nullptr};
...
const MenuEntrySub portLimitMenu = {
    ...
    [](SubMenuRef __unused sub)
    { writeSetting(PORTLIMIT_ADDR, portLimit); }, nullptr};
...
const MenuEntrySub portLoLimitMenu = {
    ...
    [](SubMenuRef __unused sub)
    { writeSetting(PORT_LO_LIM_ADDR, portLoLimit); }, nullptr};
...
const MenuEntrySub pitchBendMenu = {
    ...
    [](SubMenuRef __unused)
    { writeSetting(PB_ADDR, PBdepth); }, nullptr};
...
const MenuEntrySub extraMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(EXTRA_ADDR, extraCT); }, nullptr};
...
const MenuEntrySub extraSrcMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(EXTRA_SRC_ADDR, extraSrc); }, nullptr};
...
const MenuEntrySub extraCC2Menu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(EXTRA2_ADDR, extraCT2); }, nullptr};
...
const MenuEntrySub harmonicsMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(HARMSET_ADDR, harmSetting); }, nullptr};
...
const MenuEntrySub glissandoMenu = {
    MenuType::ESub, "GLISS", "GLISS TIME", &glissSetting, 7, 127, MenuEntryFlags::EMenuEntryWrap,
    [](SubMenuRef __unused, char *out, const char **label)
    {
      if (glissSetting >= 8)
      {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(GLISSSET_ADDR, glissSetting); }, nullptr};
...
const MenuEntrySub deglitchMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(DEGLITCH_ADDR, deglitch); }, nullptr};
...
const MenuEntrySub pinkyMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(PINKY_KEY_ADDR, pinkySetting); }, nullptr};
...
const MenuEntrySub lvlCtrlCCMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(LEVEL_CC_ADDR, levelCC); }, nullptr};
...
const MenuEntrySub fingeringMenu = {
    ...
    [](SubMenuRef __unused sub)
    { writeSetting(FINGER_ADDR, fingering); }, nullptr};
#else
const MenuEntrySub fingeringMenu = {
...
const MenuEntrySub rollerMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(ROLLER_ADDR, rollerMode); }, nullptr};
...
const MenuEntrySub lpinky3Menu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(LPINKY3_ADDR, lpinky3); }, nullptr};
...
const MenuEntrySub vibRetnMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB_RETN_ADDR, vibRetn); }, nullptr};
...
const MenuEntrySub vibSenseMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB_SENS_ADDR, vibSens); }, nullptr};
...
const MenuEntrySub vibSquelchMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB_SQUELCH_ADDR, vibSquelch); }, nullptr};
...
const MenuEntrySub vibSenseBiteMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB_SENS_BITE_ADDR, vibSensBite); }, nullptr};
...
const MenuEntrySub vibSquelchBiteMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB_SQUELCH_BITE_ADDR, vibSquelchBite); }, nullptr};
...
const MenuEntrySub vibControlMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB_CONTROL_ADDR, vibControl); }, nullptr};
...
const MenuEntrySub vib2BrDirMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB2BR_DIRECTION_ADDR, vib2BrDirection); }, nullptr};
...
const MenuEntrySub vib2BrAmtMenu = {
    ...
    [](const MenuEntrySub &__unused sub)
    { writeSetting(VIB2BR_AMOUNT_ADDR, vib2BreathAmount); }, nullptr};
