...
unsigned short fingering;          // NuRAD: 0-7 EWI,EWX,SAX,EVI,EVR,XVI,XVR,WX5; NuEVI: 0-5 EVI,EVR,TPT,HRN,XVI,XVR;  ws added WX5, XVI and XVR
...
unsigned short biteControl = 0;        // OFF, VIB, GLD, CC, GLO (ws)
...
unsigned short vib2BreathAmount = 4;   // 0-10
...
int glissInterval = 0;                  // 0,1-4        // ws: number of semitones step size between notes for glissando mode
...
void setup()
{
  ...
  { // should be D
    while (1)
    { // Touch sensor initialization failed - stop doing stuff
...
#if defined(NURAD) or defined(EVIR2)
  biteJumper = true;
#else
...
void loop()
{
  ...
  if (mainState == NOTE_OFF)
  {
    ...
    if (legacy || legacyBrAct)
    {
      ...
      if (pcCombo1 && (pcCombo1 != lastpcc1))
      { // latched note number to patch number, send with K1/K5 combo
      ...
      else if (pcCombo2 && (pcCombo2 != lastpcc2))
      { // hi and lo patch numbers, send with K2/K6 combo
        if (pitchlatch > 75)
        {
      ...
      { // both pb pads touched or br suck
        ...
        if (exSensorUse >= ((extracThrVal + extracMaxVal) / 2))
        { // instant midi setting
      ...
      {
        if (justPbDn && legacyBrAct && brSuck && programonce == false)
        { // down bend for suck programming button
          programonce = true;

          if (octaveR == 0)
          { // lowest octave position

            if (K1 && K2 && !K3 && K4)
            { //  e28 send patch change -10
              patch = patch - 10;
              doPatchUpdate = 1;
            }
            else if (K1 && !K2 && !K3 && K4)
            { // f29 decrement and send patch change
              patch--;
              doPatchUpdate = 1;
            }
            else if (!K1 && K2 && !K3 && K4)
            { // f#30 send patch change +10
              patch = patch + 10;
              doPatchUpdate = 1;
            }
            else if (!K1 && !K2 && !K3 && K4)
            { // g31 increment and send patch change
              patch++;
              doPatchUpdate = 1;
            }

            if (!K1 && !K2 && K3 && !K4)
            { // send reverb pitchlatch value
          ...
          if (octaveR == 3)
          { // middle octave position to set breath parameters
  ...
  else if (mainState == NOTE_ON)
  {
    ...
    else
    {
      if (noteValueCheck(fingeredNote) != activeNote)
      {
        ...
        activeNote = noteValueCheck(activeNote);
        if ((parallelChord || subOctaveDouble || rotatorOn) && priority)
        {                              // poly playing, send old note off before new note on
        ...
        if (!parallelChord && !subOctaveDouble && !rotatorOn)
        {                              // mono playing, send old note off after new note on
  ...
  if (dacMode == DAC_MODE_PITCH)
  { // pitch CV from DAC and breath CV from PWM on pin 6, for filtering and scaling on separate board
  ...
  else if (dacMode == DAC_MODE_BREATH)
  { // else breath CV on DAC pin, directly to unused pin of MIDI DIN jack
...
void breath()
{
  ...
  int breathFromVib = (int)(((float)vibSignal) * (float)vib2BreathAmount / 40.0f);
  ...
  if (brHarmSetting)
    ...
    brHarmonics = 0;
  if (breathCCval != oldbreath)
  { // only send midi data if breath has changed from previous value
    if (breathCC)
    {
      // send midi cc
  ...
  {
    if ((breathCC > 4) && (breathCC < 9))
    { // send high resolution midi
...
void pitch_bend()
{
  ...
  { // bite vibrato
    if (biteJumper)
    {                                            // PBITE (if pulled low with jumper, or NuRAD compile, use pressure sensor instead of capacitive bite sensor)
  ...
  { // lever vibrato
    ...
    if (allPiezo)
    { // Using the piezo voltage and zeroing directly
      if (UPWD != vibDirection)
    ...
    { // Use delta like with capacitive and bite sensor
      if (vibRead < vibThr)
      {
        if (UPWD == vibDirection)
        {
  ...
  if (pbSend != oldpb)
  { // only send midi data if pitch bend has changed from previous value
...
#if defined(NURAD)
    if (R2 && R3 && R4 && R5)
    { // fold thumb in to cover R2 through R5
...
// Extra Controller is the lip touch sensor (proportional) in front of the mouthpiece
  ...
  if (2 == stripControl)
  { // strip/aux set to CC
    if (((exSensor) >= extracThrVal))
    { // we are over the threshold, calculate CC value
  ...
  if ((extraCT || extraCT2) && (exSensorUse >= extracThrVal))
  { // if we are enabled and over the threshold, send data
    if (!extracIsOn)
    {
      extracIsOn = 1;
      if ((extraCT == 4) && CC1sw)
    ...
    if ((extraCT == 3) && (breathCC != 9) && CC1sw)
    { // Send filter cutoff (CC#74)
      extracCC = map(constrain(exSensorUse, extracThrVal, extracMaxVal), extracThrVal, extracMaxVal, 1, 127);
      if (extracCC != oldextrac)
      {
  ...
  { // we have just gone below threshold, so send zero value
    extracIsOn = 0;
    if ((extraCT == 1) && CC1sw)
    { // MW
      if (oldextrac != 0)
    ...
    else if ((extraCT == 3) && (breathCC != 9) && CC1sw)
    { // CF
      if (oldextrac != 0)
      {
...
void portamento_()
{
  ...
  if (2 == biteControl || 4 == biteControl)
  { // ws, allow gliss (GLO) too
    // Portamento is controlled with the bite sensor in the mouthpiece
    if (biteJumper)
    {                                           // PBITE (if pulled low with jumper or if on a NuRAD, use pressure sensor instead of capacitive bite sensor)
    ...
    if (portamento && (biteSensor >= portamThrVal))
    { // if we are enabled and over the threshold, send portamento
  ...
  if (1 == stripControl)
  {
    ...
    if (portamento && ((stripRead) >= extracThrVal))
    { // if we are enabled and over the threshold, send portamento
  ...
  if (portSumCC)
  { // there is a portamento level, so go for it
    if (!portIsOn)
    {
      portOn();
    }
...
void portOn()
{
  if ((portamento == 2) || (portamento == 5))
  { // if portamento midi switching is enabled
    midiSendControlChange(CCN_PortOnOff, 127);
  }
  else if (portamento == 3)
  { // if portamento midi switching is enabled - SE02 OFF/LIN
    midiSendControlChange(CCN_PortSE02, 64);
  }
  else if (portamento == 4)
  { // if portamento midi switching is enabled - SE02 OFF/EXP
...
{
  if ((portamento != 5) && (portCC != oldport))
  { // portamento setting 5 is switch only, do not transmit glide rate
...
void portOff()
{
  if ((portamento != 5) && (oldport != portLoLimit))
  { // did a zero get sent? if not, then send one (unless portamento is switch only)
...
#else
  // Bite sensor
  if (digitalRead(biteJumperPin))
  { // PBITE (if pulled low with jumper, pressure sensor is used instead of capacitive bite sensing)
    // Capacitive sensor
...
//CV filter leak prevention when putting NuEVI aside
  */
  ...
  { // EWX extended EWI fingering - lift LH1 for extended range up, touch RHp3 for extended range down
  ...
  {                                                                // EVR is EVI fingering with reversed octave rollers
    fingeredNoteUntransposed = startNote - 2 * RH1 - RH2 - 3 * RH3 //"Trumpet valves"
                               - 5 * LH1                           // Fifth key
                               + 2 * RHs + 4 * RHp3                // Trill keys +2 and +4
                               + (!LH2 || !LH3 || LHp2)            // Trill +1 achieved by lifting finger from LH2 or LH3, or touching LHp2
  ...
  {                                                                // XVI fingering (extended EVI)   ws: added XVI, XVR
    fingeredNoteUntransposed = startNote - 2 * RH1 - RH2 - 3 * RH3 //"Trumpet valves"
                               - 5 * LH1                           // Fifth key
  ...
  {                                                                // XVR is XVI fingering with reversed octave rollers
    fingeredNoteUntransposed = startNote - 2 * RH1 - RH2 - 3 * RH3 //"Trumpet valves"
                               - 5 * LH1                           // Fifth key
                               + 4 * (!LH2)                        // Maj Third up when lift
                               + 3 * (!LH3)                        // Min Third up when lift
  ...
  {    // WX5 fingering
    ...
    while (matched < 10 && combo < 27)
    {
      ...
      for (byte finger = 0; finger < 10; finger++)
      {
        if ((saxFinger[finger] == wx5FingerMatch[combo - 1][finger]) || (wx5FingerMatch[combo - 1][finger] == 2))
          matched++;
      }
    }
...
// Read touch pads (MPR121), compare against threshold value
  ...
  if (!patchKey)
    patchKey = pinkyKey = (touchValueRH[patchPinEVI] < ctouchThrVal);
  ...
  {                                                                                         // XVI fingering
    fingeredNoteUntransposed = startNote - 2 * K1 - K2 - 3 * K3                             //"Trumpet valves" (only count if the corresponding trill key is not also pressed)
                               - 5 * K4                                                     // Fifth key
      ...
      if ((millis() - lastGlissTime) >= glissTime)
      {
        if (glissNoteDiff < 0) // current higher than target
        {
          glissCurrentNote -= (abs(glissNoteDiff) >= abs(glissInterval))
                                  ? glissInterval
                              : abs(glissTargetInterval) > abs(glissInterval) ? 1
                                                                              : abs(glissNoteDiff);
        }
